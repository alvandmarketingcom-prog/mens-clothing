  ...
  useEffect(() => {
    ...
    const setupScrollTrigger = (scrollDistance, onProgress) => {
      if (hintRef.current) {
        ...
        cueTween = gsap.to(hintRef.current, {
          y: 10,
          duration: 0.9,
          ease: 'power1.inOut',
          yoyo: true,
    ...
    const initFrames = async () => {
      ...
      // Load ALL frames before unlocking scroll (user request)
      let loaded = 0;
      const BATCH = 6;
      for (let start = 1; start <= FRAME_COUNT; start += BATCH) {
        if (cancelled) return;
        const end = Math.min(FRAME_COUNT, start + BATCH - 1);
        const jobs = [];
          ...
          jobs.push(
            loadImage(FRAME_PATH(i)).then((img) => {
              if (img && img.naturalWidth > 0) {
                ...
                // keep slot empty — drawFrame will fall back to nearest good frame / poster
                framesCacheRef.current[i - 1] = null;
              }
              loaded += 1;
              if (!cancelled) {
                setLoadPct(Math.min(99, Math.round((loaded / FRAME_COUNT) * 100)));
              }
            })
          );
        }
        ...
        // Show first frame as soon as we have it
        if (start === 1 && frameImgRef.current) {
      ...
      const scrollDistance = Math.max(FRAME_COUNT * 28, 2000);
  ...
  return (
    <>
      ...
      {!heroReady && (
        ...
        >
          ...
          >
            ...
            >
              {Math.max(0, Math.min(100, loadPct))}%
            </p>
          </div>
        </div>
      )}
      ...
      >
        ...
        <div ref={hintRef} className="hero-scroll-cue">
        ...
        <div ref={body1Ref} className="hero-body-overlay">
          <p className="hero-body-text hero-body-text--split">
            <span className="hero-body1-line1">مرکز تخصصی فروش</span>
        ...
        <div ref={body2Ref} className="hero-body-overlay">
          <p className="hero-body-text">
            <span className="hero-body-line1">تنوع در</span>
            <span className="hero-body-space"> </span>
            <span className="hero-body-line2">قیمت و کیفیت</span>
          </p>
        </div>
      </section>
