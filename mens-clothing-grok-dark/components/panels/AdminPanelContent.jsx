...
export default function AdminPanelContent() {
  ...
  return (
    <>
            <div className="admin-panel-shell w-full min-h-screen flex flex-col bg-primary-50 dark:bg-primary-950 overflow-x-hidden">
            <div className="w-full max-w-7xl mx-auto px-3 sm:px-4 py-6 sm:py-10 pb-24 flex-1">
              <div className="flex flex-wrap items-center justify-between gap-4 mb-6 p-4 sm:p-5 rounded-2xl border border-primary-200 dark:border-white/20 bg-white dark:bg-primary-900">
                <div className="flex items-center gap-3">
                  <div className="w-14 h-14 rounded-full bg-primary-800 dark:bg-[#FF6B35] text-white flex items-center justify-center text-xl font-bold">{(adminUser.name || 'ا')[0]}</div>
                  <div>
                  ...
                  {!adminLoading && adminTab === 'dashboard' && (
                    <div className="space-y-6">
                      ...
                      <div className="p-4 rounded-2xl border border-primary-200 dark:border-white/15 bg-white dark:bg-primary-900 space-y-3">
                        <div>
                          ...
                          <p className="text-xs text-primary-500 dark:text-white/60 mt-1 leading-6">
                            چون بک‌اند ندارید، تنظیمات و کاتالوگ روی همین دستگاه ذخیره می‌شود. خروجی JSON بگیرید تا بعداً بازیابی کنید.
                          </p>
                        </div>
                        <div className="flex flex-wrap gap-2">
                          <button
                            ...
                            onClick={() => {
                              try {
                                downloadBackupFile(`pirahanmardane-backup-${Date.now()}.json`);
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                        {[
                          { label: 'سفارش امروز', value: toFa((adminOrders||[]).filter(o=>o.date?.includes('۰۵/۲۰')).length || 1) },
                          { label: 'سفارش هفته', value: toFa((adminOrders||[]).length) },
                          { label: 'فروش ماه (تومان)', value: toFa(Math.round(((adminOrders||[]).reduce((s,o)=>s+(o.total||0),0))/1000)) + 'هزار' },
                          { label: 'فروشندگان فعال', value: toFa((adminSellers||[]).filter(s=>s.status==='approved').length) },
                          { label: 'در انتظار تأیید', value: toFa((adminSellers||[]).filter(s=>s.status==='pending').length) },
                          { label: 'محصولات فعال', value: toFa((adminProducts||[]).filter(p=>p.status==='active').length) },
                          { label: 'محصول در انتظار', value: toFa((adminProducts||[]).filter(p=>p.status==='pending').length) },
                          { label: 'تیکت باز', value: toFa((adminTickets||[]).filter(t=>t.status==='open'||t.status==='pending').length) },
                          { label: 'خریداران', value: toFa((adminBuyers||[]).length) },
                        ].map((c,i) => (
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {(adminSellers||[]).filter(s=>{
                          ...
                          <div key={s.id} className="p-3 sm:p-4 rounded-2xl border border-primary-200 dark:border-white/15 bg-white dark:bg-primary-900 flex flex-col sm:flex-row sm:items-center gap-3">
                            <button type="button" onClick={()=>setAdminSellerDetailId(s.id)} className="flex-1 text-right min-w-0">
                              <p className="font-medium text-primary-900 dark:text-white">{s.shopName}</p>
                              <p className="text-xs text-primary-500 dark:!text-white">{s.ownerName} · <span dir="ltr">{s.phone}</span> · {s.city}</p>
                              <p className="text-xs text-primary-400 mt-0.5">{toFa(s.productsCount||0)} محصول · {toFa(s.ordersCount||0)} سفارش · امتیاز {toFa(s.rating||0)}</p>
                            </button>
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className={`text-xs px-2 py-0.5 rounded-full ${adminStatusBadge(s.status)}`}>{adminStatusLabel(s.status)}</span>
                              {s.status==='pending'&&(<>
                                <button type="button" onClick={()=>saveAdminSellers((adminSellers||[]).map(x=>x.id===s.id?{...x,status:'approved'}:x))} className="text-xs px-2.5 py-1 rounded-full bg-emerald-600 text-white">تأیید</button>
                                <button type="button" onClick={()=>{sitePrompt('دلیل رد (اختیاری):').then((r)=>{saveAdminSellers((adminSellers||[]).map(x=>x.id===s.id?{...x,status:'rejected',rejectReason:r||''}:x));});}} className="text-xs px-2.5 py-1 rounded-full bg-red-500 text-white">رد</button>
                              </>)}
                              {s.status==='approved'&&<button type="button" onClick={()=>saveAdminSellers((adminSellers||[]).map(x=>x.id===s.id?{...x,status:'blocked'}:x))} className="text-xs px-2.5 py-1 rounded-full border border-red-200 text-red-600">مسدود</button>}
                              {s.status==='blocked'&&<button type="button" onClick={()=>saveAdminSellers((adminSellers||[]).map(x=>x.id===s.id?{...x,status:'approved'}:x))} className="text-xs px-2.5 py-1 rounded-full border border-emerald-200 text-emerald-600">رفع مسدودی</button>}
                            </div>
                          </div>
                        ))}
                        {!(adminSellers||[]).length && <p className="text-center text-sm text-primary-400 py-10">فروشنده‌ای ثبت نشده</p>}
                      </div>
                    </div>
                  )}
                  {!adminLoading && adminTab === 'sellers' && adminSellerDetailId && (()=>{
                    const s=(adminSellers||[]).find(x=>x.id===adminSellerDetailId);
                    if(!s) return null;
                    const sellerProds=(adminProducts||[]).filter(p=>p.sellerId===s.id);
                    return (
                      <div>
                        <button type="button" onClick={()=>setAdminSellerDetailId(null)} className="text-xs text-apple-blue mb-4 flex items-center gap-1">← بازگشت به لیست</button>
                        <h2 className="text-base font-bold text-primary-900 dark:text-white mb-4">{s.shopName}</h2>
                        <div className="grid sm:grid-cols-2 gap-4 mb-6 text-sm">
                          <div className="p-4 rounded-2xl bg-white dark:bg-primary-900 border border-primary-200 dark:border-white/15 space-y-2">
                            <p><span className="text-primary-500">مسئول:</span> {s.ownerName}</p>
                            <p><span className="text-primary-500">موبایل:</span> <span dir="ltr">{s.phone}</span></p>
                            <p><span className="text-primary-500">شهر:</span> {s.city}</p>
                            <p><span className="text-primary-500">عضویت:</span> {s.joinDate}</p>
                            <p><span className="text-primary-500">شبا:</span> <span dir="ltr" className="text-xs">{s.sheba||'—'}</span></p>
                            <p><span className="text-primary-500">درباره:</span> {s.about||'—'}</p>
                            <p><span className={`text-xs px-2 py-0.5 rounded-full ${adminStatusBadge(s.status)}`}>{adminStatusLabel(s.status)}</span></p>
                            <div className="pt-2 border-t border-primary-100 dark:border-white/10 mt-2">
                              <label className="flex items-center justify-between gap-2 cursor-pointer">
                                <span className="text-xs text-primary-600 dark:text-white/70">تأیید جواز و فعال‌سازی فروش</span>
                                <button
                                  type="button"
                                  onClick={() => {
                                    const next = !(s.licenseApproved === true);
                                    saveAdminSellers((adminSellers || []).map((x) => x.id === s.id ? { ...x, licenseApproved: next, licenseRequired: true } : x));
                                    try {
                                      const su = JSON.parse(localStorage.getItem('sellerUser') || 'null');
                                      if (su && (su.id === s.id || su.shopName === s.shopName || (s.id === 'own' && (su.id === 'own' || !su.id)))) {
                                        const updated = { ...su, licenseApproved: next, licenseRequired: true };
                                        localStorage.setItem('sellerUser', JSON.stringify(updated));
                                        if (typeof saveSellerUser === 'function') saveSellerUser(updated);
                                      }
                                    } catch (_) {}
                                    showToast({
                                      message: next ? 'فروشنده مجاز به فروش شد · پنل فعال' : 'مجوز فروش لغو شد · پنل فروش غیرفعال',
                                  ...
                                  dir="ltr"
                                  className={`relative inline-flex h-7 w-12 items-center rounded-full p-0.5 transition-colors duration-300 ease-out focus:outline-none focus-visible:ring-2 focus-visible:ring-apple-blue/40 ${s.licenseApproved === true ? 'bg-apple-blue' : 'bg-primary-300 dark:bg-primary-600'}`}
                                >
                                  <span className={`pointer-events-none inline-block h-6 w-6 rounded-full bg-white shadow-md transition-transform duration-300 ease-out ${s.licenseApproved === true ? 'translate-x-5' : 'translate-x-0'}`} />
                                </button>
                              </label>
                              <p className="text-xs text-primary-400 mt-1">
                                {s.licenseApproved === true
                                  ? 'جواز تأیید شده · فروشنده می‌تواند بفروشد و پنل فعال است'
                                  : 'جواز اجباری است · تا تأیید ادمین، فروش و پنل غیرفعال است'}
                              </p>
                            </div>
                          </div>
                          <div className="p-4 rounded-2xl bg-white dark:bg-primary-900 border border-primary-200 dark:border-white/15">
                            <p className="text-xs font-bold mb-2">آمار</p>
                            <p className="text-sm">{toFa(s.productsCount||sellerProds.length)} محصول · {toFa(s.ordersCount||0)} سفارش · امتیاز {toFa(s.rating||0)}</p>
                            {s.id && topSellers.some(t=>t.id===s.id) && (
                              <button type="button" onClick={()=>{closeAdminPanel(); openSeller(s.id);}} className="mt-3 text-xs px-3 py-1.5 rounded-full border border-primary-200 dark:border-white/30">مشاهده صفحه عمومی فروشنده</button>
                            )}
                          </div>
                        </div>
                        <h3 className="text-sm font-bold mb-2">محصولات این فروشنده</h3>
                        <div className="space-y-2">
                          {sellerProds.map(p=>(
                            <div key={p.id} className="flex items-center gap-3 p-2 rounded-xl bg-white dark:bg-primary-900 border border-primary-100 dark:border-white/10">
                              {p.image && <img src={p.image} alt="" className="w-12 h-14 object-cover rounded-lg" />}
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-medium truncate">{p.name}</p>
                                <p className="text-xs text-primary-500">{toFa((p.price||0).toLocaleString())} ت · موجودی {toFa(p.stock||0)}</p>
                              </div>
                    ...
                    <div>
                      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 w-full sm:w-auto">
                          <button type="button" onClick={() => backupAdminProducts('json')} className="text-xs px-3 py-1.5 rounded-full border border-primary-200 dark:border-white/20 text-primary-700 dark:text-white font-medium flex items-center gap-1 hover:bg-primary-50 dark:hover:bg-primary-800" title="بک‌آپ کامل JSON با تمام اطلاعات"><Icon name="download" size={14} /> بک‌آپ JSON</button>
                          <button type="button" onClick={() => backupAdminProducts('csv')} className="text-xs px-3 py-1.5 rounded-full border border-primary-200 dark:border-white/20 text-primary-700 dark:text-white font-medium flex items-center gap-1 hover:bg-primary-50 dark:hover:bg-primary-800" title="خروجی جدول CSV"><Icon name="download" size={14} /> CSV</button>
                          <label className="text-xs px-3 py-1.5 rounded-full border border-primary-200 dark:border-white/20 text-primary-700 dark:text-white font-medium flex items-center gap-1 hover:bg-primary-50 dark:hover:bg-primary-800 cursor-pointer" title="بازگردانی از فایل JSON">
                            <Icon name="upload" size={14} /> بازگردانی
                            <input type="file" accept="application/json,.json" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) restoreAdminProductsFromFile(f); e.target.value = ''; }} />
                          </label>
                          <label className="text-xs px-3 py-1.5 rounded-full border border-emerald-300 text-emerald-700 dark:text-emerald-300 font-medium flex items-center gap-1 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 cursor-pointer" title="ورود از CSV ووکامرس یا شاپیفای">
                            <Icon name="upload" size={14} /> ورود Woo/Shopify
                            <input type="file" accept=".csv,text/csv" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) importExternalProductsCsv(f, 'admin'); e.target.value = ''; }} />
                          </label>
                      ...
                      <div className="flex gap-1 overflow-x-auto no-scrollbar mb-4">
                        {[{id:'all',l:'همه'},{id:'active',l:'فعال'},{id:'pending',l:'در انتظار'},{id:'rejected',l:'رد‌شده'},{id:'inactive',l:'غیرفعال'}].map(f=>(
                          <button key={f.id} type="button" onClick={()=>setAdminProductFilter(f.id)} className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium border ${adminProductFilter===f.id?'bg-primary-800 text-white border-primary-800 dark:bg-[#FF6B35] dark:!text-white':'plp-filter-chip border-primary-300 dark:border-white/50 !text-primary-900 dark:!text-white bg-white dark:bg-[#2A2C30] font-medium'}`}>{f.l}</button>
                        ))}
                      </div>
                      <div className="space-y-2">
                        {(adminProducts||[]).filter(p=>{
                          if(adminProductFilter!=='all'&&p.status!==adminProductFilter) return false;
                          const q=adminProductSearch.trim().toLowerCase();
                          if(!q) return true;
                          return (p.name||'').toLowerCase().includes(q)||(p.sellerName||'').toLowerCase().includes(q);
                        }).map(p=>(
                          <div key={p.id} className="flex items-center gap-3 p-3 rounded-2xl border border-primary-200 dark:border-white/15 bg-white dark:bg-primary-900">
                            {p.image && <img src={p.image} alt="" className="w-14 h-16 object-cover rounded-lg flex-shrink-0" />}
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-medium text-primary-900 dark:text-white truncate">{p.name}</p>
                              <p className="text-xs text-primary-500">{p.sellerName} · {toFa((p.price||0).toLocaleString())} ت · موجودی {toFa(p.stock||0)}</p>
                              {p.productCode && <p className="text-xs text-primary-400 mt-0.5" dir="ltr">کد: {p.productCode}</p>}
                              {p.status==='active' && p.productCode && (
                                <button type="button" onClick={() => copyTextToClipboard(getProductPublicUrl(p))} className="mt-1 text-xs text-apple-blue hover:underline flex items-center gap-1"><Icon name="copy" size={14} /> کپی لینک منتشرشده</button>
                              )}
                            </div>
                            <div className="flex flex-col items-end gap-1">
                              ...
                              <div className="flex gap-1">
                                {p.status==='pending'&&(<>
                                  <button type="button" onClick={()=>approveAdminProduct(p.id)} className="text-xs px-2 py-0.5 rounded-full bg-emerald-600 text-white">تأیید</button>
                                  <button type="button" onClick={()=>{sitePrompt('دلیل رد:').then((r)=>{saveAdminProducts((adminProducts||[]).map(x=>x.id===p.id?{...x,status:'rejected',rejectReason:r||''}:x));});}} className="text-xs px-2 py-0.5 rounded-full bg-red-500 text-white">رد</button>
                                </>)}
                                {p.status==='active'&&<button type="button" onClick={()=>saveAdminProducts((adminProducts||[]).map(x=>x.id===p.id?{...x,status:'inactive'}:x))} className="text-xs px-2 py-0.5 rounded-full border border-primary-200 text-primary-600">غیرفعال</button>}
                                {p.status==='inactive'&&<button type="button" onClick={()=>saveAdminProducts((adminProducts||[]).map(x=>x.id===p.id?{...x,status:'active'}:x))} className="text-xs px-2 py-0.5 rounded-full border border-emerald-200 text-emerald-600">فعال</button>}
                                <button type="button" onClick={()=>{siteConfirm('حذف این محصول؟').then(ok=>{if(ok)saveAdminProducts((adminProducts||[]).filter(x=>x.id!==p.id));});}} className="text-xs px-2 py-0.5 rounded-full border border-red-200 text-red-600">حذف</button>
                              </div>
                            </div>
                          </div>
                        ))}
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {(adminTickets||[]).filter(t=>adminTicketFilter==='all'||t.status===adminTicketFilter).map(t=>(
                          <button key={t.id} type="button" onClick={()=>{setAdminTicketDetailId(t.id);setAdminTicketReply('');if(t.unread)saveAdminTickets((adminTickets||[]).map(x=>x.id===t.id?{...x,unread:false}:x));}} className="w-full p-3 rounded-2xl border border-primary-200 dark:border-white/15 bg-white dark:bg-primary-900 text-right flex items-center justify-between gap-3">
                            <div className="min-w-0">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                              <button type="button" onClick={()=>{
                                ...
                                const nextAdm = (adminTickets||[]).map(x => x.id===t.id ? { ...x, status: 'pending', messages: [...(x.messages||[]), msg], unread: false } : x);
                                saveAdminTickets(nextAdm);
                                // همگام با گفتگوی خریدار + ویجت چت
                                try {
                                  const bt = Array.isArray(buyerTickets) ? [...buyerTickets] : JSON.parse(localStorage.getItem('buyerTickets') || '[]');
                                  if (bt.some(x => x.id === t.id)) {
                                    const nextBt = bt.map(x => x.id === t.id ? { ...x, status: 'open', messages: [...(x.messages||[]), msg] } : x);
                                    saveBuyerTickets(nextBt);
                                    const conv = nextBt.find(x => x.id === t.id);
                              ...
                              <button type="button" onClick={()=>saveAdminTickets((adminTickets||[]).map(x=>x.id===t.id?{...x,status:'closed'}:x))} className="px-4 py-2 rounded-full border border-primary-200 text-sm">بستن تیکت</button>
                            </div>
                          </div>
                        )}
                        ...
                        {(adminBuyers||[]).filter(b=>{
                          ...
                          <div key={b.id} className="p-3 rounded-2xl border border-primary-200 dark:border-white/15 bg-white dark:bg-primary-900 flex items-center justify-between gap-3">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {b.status==='active'?<button type="button" onClick={()=>saveAdminBuyers((adminBuyers||[]).map(x=>x.id===b.id?{...x,status:'blocked'}:x))} className="text-xs px-2 py-1 rounded-full border border-red-200 text-red-600">مسدود</button>:
                              <button type="button" onClick={()=>saveAdminBuyers((adminBuyers||[]).map(x=>x.id===b.id?{...x,status:'active'}:x))} className="text-xs px-2 py-1 rounded-full border border-emerald-200 text-emerald-600">رفع مسدودی</button>}
                            </div>
                          </div>
                        ))}
                    ...
                    return (
                      <div>
                        ...
                        <div className="p-4 rounded-2xl border border-primary-200 dark:border-white/15 bg-white dark:bg-primary-900 mb-4 space-y-2">
                          ...
                          <p className="text-xs text-primary-600 dark:text-white/70">عضویت: {b.joinDate} · سفارش‌ها: {toFa(b.ordersCount||0)} · وضعیت: {b.status==='blocked'?'مسدود':'فعال'}</p>
                          ...
                          {(adminCategories || []).map((c) => (
                            <div key={c.id} className="flex flex-wrap items-center gap-3 p-3 rounded-xl border border-primary-200 dark:border-white/15 bg-white dark:bg-primary-900">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-primary-500 dark:!text-white mt-0.5 font-latin" dir="ltr">{c.url || `/shop?cat=${c.slug}`}</p>
...
{/* Brands catalog */}
                  ...
                  {!adminLoading && adminTab === 'pages' && (
                    <div className="space-y-4 max-w-3xl">
                      ...
                      {(() => {
                        const base = (seoCfg().canonicalBase || 'https://pirahanmardane.ir').replace(/\/$/, '');
                        const sitePages = [
                    ...
                    <div className="space-y-6">
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                          ...
                          <p className="text-xs text-primary-500 mt-0.5">روش‌های فعال برای انتخاب فروشنده و خریدار در تسویه</p>
                        </div>
                        <button
                          type="button"
                          ...
                          <div key={m.id} className="p-4 rounded-2xl border border-primary-200 dark:border-white/15 bg-white dark:bg-primary-900 space-y-3">
                            <div className="flex flex-wrap items-center justify-between gap-2">
                              <div>
                                ...
                                <p className="text-xs text-primary-500">{m.priceMode === 'dynamic_cod' ? 'قیمت داینامیک · تسویه در مقصد' : `مبلغ پایه: ${toFa((m.baseCost||0).toLocaleString())} ت`} · {m.eta}</p>
                              </div>
                              <div className="flex items-center gap-2">
                            ...
                            <div className="space-y-3">
                              ...
                              <div>
                                ...
                                <select value={shippingMethodForm.priceMode||'fixed'} onChange={e => setShippingMethodForm(f => ({...f, priceMode: e.target.value}))} className="w-full px-3 py-2.5 rounded-xl border border-primary-200 dark:border-white/20 bg-transparent text-sm text-primary-900 dark:text-white">
                                  <option value="fixed">ثابت (در تسویه)</option>
                        ...
                        <div className="site-modal-root" role="dialog" aria-modal="true">
                          ...
                          <div className="site-modal-panel bg-white dark:bg-primary-900 p-5 border border-primary-200 dark:border-white/15">
                            {(() => {
                              ...
                              const prefix = t === 'category' ? '/shop?cat=' : t === 'tag' ? '/shop?tag=' : t === 'brand' ? '/brands/' : t === 'blog-category' ? '/blog?cat=' : '/blog?tag=';
                            ...
                            <div key={item.id} className="p-4 rounded-2xl border border-primary-200 dark:border-white/15 bg-white dark:bg-primary-900 flex flex-wrap gap-3 items-start">
                              ...
                              <div className="flex gap-2">
                                ...
                                <button type="button" onClick={() => {
                                  ...
                                  if (item.productId) saveSellerProducts((sellerProducts||[]).map(pr => pr.id===item.productId ? { ...pr, status: 'inactive', contentStatus: 'rejected' } : pr));
                  ...
                  {!adminLoading && adminTab === 'blog' && (
                    <div className="space-y-4">
                      ...
                      {blogForm && blogForm.id && (
                        <div className="p-4 rounded-2xl border border-apple-blue/40 dark:border-[#FF6B35]/40 bg-white dark:bg-primary-900 space-y-2">
                      ...
                      <div className="space-y-2">
                        ...
                        {(blogPosts || []).map(p => (
                          <div
                            key={p.id}
                            className={`flex flex-wrap items-center gap-2 p-3 rounded-xl border bg-white dark:bg-primary-900 transition ${blogForm && blogForm.id === p.id ? 'border-apple-blue dark:border-[#FF6B35] ring-1 ring-apple-blue/30' : 'border-primary-200 dark:border-white/15'}`}
                          >
                            <button
                              type="button"
                      ...
                      <div className="space-y-2">
                        {campaignsList.map(c => (
                          <div key={c.id} className="flex flex-wrap items-center gap-2 p-3 rounded-xl border border-primary-200 dark:border-white/15 bg-white dark:bg-primary-900">
                            ...
                            {c.status === 'pending' && (
                              <button type="button" onClick={() => saveCampaigns(campaignsList.map(x => x.id === c.id ? { ...x, status: 'approved', active: true } : x))} className="text-xs px-2.5 py-1 rounded-full bg-emerald-500 text-white">تأیید</button>
                            )}
                    ...
                    <div className="space-y-6 max-w-3xl">
                      ...
                      <div className="p-4 rounded-2xl border border-primary-200 dark:border-white/15 bg-white dark:bg-primary-900 space-y-4">
                        <div>
                          ...
                          <p className="text-xs text-primary-500 mt-1">برای هر صفحه: تحلیل، کلمات کلیدی، Title، Meta، FAQ، پیش‌نمایش گوگل/شبکه، سپس ایندکس و کنونیکال. سئوی محصول در پنل فروشنده است.</p>
                        </div>
                        {(() => {
                          const base = (seoCfg().canonicalBase || 'https://pirahanmardane.ir').replace(/\/$/, '');
                          const staticPages = [
                          ...
                          return (
                            <div className="space-y-3">
                              ...
                              <p className="text-[10px] text-primary-400 leading-relaxed">سئوی محصول از پنل فروشنده · سئوی دسته/برند هنگام ساخت همان آیتم نیز در دسترس است · ایندکس سراسری حذف شد؛ ایندکس هر صفحه اینجاست.</p>
                            </div>
                          );
                        })()}
                      </div>
                  ...
                  {!adminLoading && adminTab === 'redirects' && (
                    <div className="space-y-5 max-w-3xl">
                      ...
                      <div className="p-4 rounded-2xl border border-primary-200 dark:border-white/15 bg-white dark:bg-primary-900 space-y-3">
                                ...
                                {(seoRedirects || []).map(r => (
                                  <tr key={r.id} className="border-t border-primary-100 dark:border-white/10">
                                    ...
                                    <td className="p-2 whitespace-nowrap">
                                      <button
                                        type="button"
                                        className="text-apple-blue dark:text-[#FF6B35] me-2"
                        ...
                        <p className="text-[10px] text-primary-400 leading-relaxed">در مرورگر فعلی از localStorage اعمال می‌شود. برای سرور، «خروجی JSON» را در public/seo-redirects.json بگذارید یا API مربوط را وصل کنید.</p>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="space-y-4 max-w-5xl">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="text-xs text-primary-500 mt-1 font-latin" dir="ltr">{gscStore?.property || 'https://pirahanmardane.ir/'}</p>
                        </div>
                        <div className="flex flex-wrap gap-1.5">
                        ...
                        <div className="space-y-4">
                          <div className="p-4 rounded-2xl border border-primary-200 dark:border-white/15 bg-white dark:bg-primary-900 space-y-3">
                            ...
                            <div className="flex flex-col sm:flex-row gap-2">
                              <input value={adminGscInspectUrl} onChange={e => setAdminGscInspectUrl(e.target.value)} dir="ltr" placeholder="https://pirahanmardane.ir/shop یا /shop" className="flex-1 px-3 py-2.5 rounded-xl border border-primary-200 dark:border-white/20 bg-transparent text-sm text-left font-latin text-primary-900 dark:text-white" />
                      ...
                      {sub === 'sitemaps' && (
                        <div className="space-y-4">
                          <div className="p-4 rounded-2xl border border-primary-200 dark:border-white/15 bg-white dark:bg-primary-900 space-y-3">
                            ...
                            <div className="flex flex-col sm:flex-row gap-2">
                              <input id="gsc-sitemap-add" dir="ltr" placeholder="https://pirahanmardane.ir/sitemap.xml" className="flex-1 px-3 py-2.5 rounded-xl border border-primary-200 dark:border-white/20 bg-transparent text-sm text-left font-latin" />
                              <button type="button" onClick={() => {
                                ...
                                <tbody>
                                  {(gscStore?.sitemaps || []).map((s, i) => (
                                    <tr key={i} className="border-t border-primary-50 dark:border-white/5">
                                      ...
                                      <td className="p-2 text-emerald-600">{s.status}</td>
                            ...
                            <div className="max-h-72 overflow-y-auto space-y-1">
                              {liveEvents.slice(0, 50).map((e, i) => (
                                <div key={i} className="flex gap-2 text-[11px] border-b border-primary-50 dark:border-white/5 py-1.5">
                                  ...
                                  <span className="font-medium text-apple-blue dark:text-[#FF6B35] w-28 flex-shrink-0">{e.name}</span>
